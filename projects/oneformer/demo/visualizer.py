...
def _safe_to_rgb(color):
    try:
        rgb = np.asarray(mplc.to_rgb(color), dtype="float32")
    except ValueError:
        rgb = np.asarray(color, dtype="float32")[:3]
        if rgb.max() > 2:
            rgb = rgb / 255.0
...
class Visualizer:
    ...
    def draw_text(
        ...
        *,
        ...
        color = np.maximum(_safe_to_rgb(color), 0.2)
    ...
    def _change_color_brightness(self, color, brightness_factor):
        ...
        color = _safe_to_rgb(color)
        polygon_color = colorsys.rgb_to_hls(*color)
