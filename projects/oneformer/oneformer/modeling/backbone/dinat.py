...
def build_neighborhood_attention(dim, kernel_size, dilation, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop):
    attn_params = inspect.signature(NeighborhoodAttention.__init__).parameters
    attn_kwargs = {
    ...
    if "attn_drop" in attn_params:
        attn_kwargs["attn_drop"] = attn_drop
    return NeighborhoodAttention(dim, **attn_kwargs)
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7, dilation=None,
        ...
        self.attn = build_neighborhood_attention(
            dim, kernel_size, dilation, num_heads, qkv_bias, qk_scale, attn_drop, drop
        )
